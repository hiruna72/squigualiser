...
# GUPPY="/media/hiruna/data/slow5_work/guppy_integration/slow5-guppy_6.3.7/bin"
...
[ "${REF}" ] || die "edit the reference genome path (variable REF) in the script"
...
basecall() {
	...
	buttery-eel -g ${GUPPY} --config ${MODEL_TO_USE} -i ${SIGNAL_FILE} -o ${BASECALL_DIR}/moves.sam  --moves_out --port 5558 --use_tcp || die "buttery-eel failed"
	${SAMTOOLS} view ${BASECALL_DIR}/moves.sam -o ${MOVES_BAM}  || die "samtools merge failed"
	deactivate
}
