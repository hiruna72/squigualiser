...
REF_REGION="chr1:92,779,392-92,779,440"
SIM_REGION="sim_ref:1-41"
...
simulate_ref_signal() {
	...
	${SQUIGULATOR} --seed 1 --full-contigs --ideal-time --amp-noise 0 -x ${SIMULATING_PROFILE} ref_SNV_added.fasta -o ${SIMULATE_REF_DIR}/sim_SNV_added.slow5 -c ${SIMULATE_REF_DIR}/sim_SNV_added.paf -a ${SIMULATE_REF_DIR}/sim_SNV_added.sam -q ${SIMULATE_REF_DIR}/sim_SNV_added.fasta || die "squigulator failed"
	${SAMTOOLS} sort ${SIMULATE_REF_DIR}/sim_SNV_added.sam -o ${SIMULATE_REF_DIR}/sim_SNV_added.bam || die "samtools sort failed"
	${SAMTOOLS} index ${SIMULATE_REF_DIR}/sim_SNV_added.bam || die "samtools index failed"

	${SQUIGULATOR} --seed 1 --full-contigs --ideal-time --amp-noise 0 -x ${SIMULATING_PROFILE} ref_reverse.fasta -o ${SIMULATE_REF_DIR}/sim_reverse.slow5 -c ${SIMULATE_REF_DIR}/sim_reverse.paf -a ${SIMULATE_REF_DIR}/sim_reverse.sam -q ${SIMULATE_REF_DIR}/sim_reverse.fasta || die "squigulator failed"
	${SAMTOOLS} sort ${SIMULATE_REF_DIR}/sim_reverse.sam -o ${SIMULATE_REF_DIR}/sim_reverse.bam || die "samtools sort failed"
	${SAMTOOLS} index ${SIMULATE_REF_DIR}/sim_reverse.bam || die "samtools index failed"

	${SQUIGULATOR} --seed 1 --full-contigs --ideal-time --amp-noise 0 -x ${SIMULATING_PROFILE} ref_SNV_added_reverse.fasta -o ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.slow5 -c ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.paf -a ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.sam -q ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.fasta || die "squigulator failed"
	${SAMTOOLS} sort ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.sam -o ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.bam || die "samtools sort failed"
	${SAMTOOLS} index ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.bam || die "samtools index failed"
...
plot_realign_and_sim() {
	...
	echo "squigualiser plot_pileup -f ref_SNV_added.fasta -s ${SIMULATE_REF_DIR}/sim_SNV_added.slow5 -a ${SIMULATE_REF_DIR}/sim_SNV_added.bam --region ${SIM_REGION} --tag_name ${MODEL_TO_USE}_sim_SNV_added --no_overlap --profile ${PROFILE_TO_DETERMINE_BASE_SHIFT}" >> ${TRACK_COMMAND_FILE}
...
plot_eventalign_reverse() {
	...
	echo "squigualiser plot_pileup -f ref_reverse.fasta -s ${SIMULATE_REF_DIR}/sim_reverse.slow5 -a ${SIMULATE_REF_DIR}/sim_reverse.bam --region ${SIM_REGION} --tag_name ${MODEL_TO_USE}_sim_read_reverse --no_overlap --profile ${PROFILE_TO_DETERMINE_BASE_SHIFT}" >> ${TRACK_COMMAND_FILE}
	echo "squigualiser plot_pileup -f ref_SNV_added_reverse.fasta -s ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.slow5 -a ${SIMULATE_REF_DIR}/sim_SNV_added_reverse.bam --region ${SIM_REGION} --tag_name ${MODEL_TO_USE}_sim_SNV_added_reverse --no_overlap --profile ${PROFILE_TO_DETERMINE_BASE_SHIFT}" >> ${TRACK_COMMAND_FILE}

	cat ${TRACK_COMMAND_FILE}

	TESTCASE="${MODEL_TO_USE}_evligned_reverse"
	OUTPUT="${OUTPUT_DIR}/testcase_${TESTCASE}"
	${PLOT_TRACK_TOOL} --shared_x -f ${TRACK_COMMAND_FILE} -o ${SQUIG_PLOT_DIR}/${TESTCASE} --tag_name ${TESTCASE} || die "testcase:$TESTCASE failed"
...
plot_realign_reverse() {
	...
	TESTCASE="${MODEL_TO_USE}_realigned_reverse"
	OUTPUT="${OUTPUT_DIR}/testcase_${TESTCASE}"
	${PLOT_TRACK_TOOL} --shared_x -f ${TRACK_COMMAND_FILE} -o ${SQUIG_PLOT_DIR}/${TESTCASE} --tag_name ${TESTCASE} || die "testcase:$TESTCASE failed"
