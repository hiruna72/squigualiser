...
M6ANET_ENV_PATH="/media/hiruna/data/basecalling_work/pore_model_results/squigle_plots/methylation/m6anet/venv3"
...
MAPPED_BAM="${OUTPUT_DIR}/map.bam"

EVENTALIGN_BAM="${OUTPUT_DIR}/eventalign.bam"
EVENTALIGN_TSV="${OUTPUT_DIR}/eventalign.tsv"
EVENTALIGN_SUM="${OUTPUT_DIR}/eventalign.summary"

M6ANET_DATAPREP="${OUTPUT_DIR}/m6anet_dataprep"
M6ANET_INFERENCE="${OUTPUT_DIR}/m6anet_inference"

METH_BED="${OUTPUT_DIR}/meth.bed"
METH_THRESHOLD=0.85

READ_ID_LIST_MODIFIED=${OUTPUT_DIR}/read_id_mod.list
READ_ID_LIST_NOT_MODIFIED=${OUTPUT_DIR}/read_id_umod.list
READ_ID_LIST_TO_PLOT=${OUTPUT_DIR}/read_id_to_plot.list
...
basecall_fast5() {
	...
	${SLOW5TOOLS} s2f ${SIGNAL_FILE} -d "${OUTPUT_DIR}/fast5"
	${GUPPY}/guppy_basecaller ${CHUNK_SIZE} -x cuda:0 --config ${MODEL_TO_USE} -i "${OUTPUT_DIR}/fast5" -s ${BASECALL_DIR} || die "guppy_basecaller failed"
...
f5c_eventalign() {
	...
	f5c eventalign -b ${MAPPED_BAM} -r ${SEQUENCE_FILE} -g ${REFERENCE} --slow5 ${SIGNAL_FILE} -o ${EVENTALIGN_TSV} --summary ${EVENTALIGN_SUM} --signal-index --scale-events
}

run_m6anet() {
	info "running m6anet..."
	
	source ${M6ANET_ENV_PATH}/bin/activate || die "could not activate m6anet venv"
	m6anet dataprep --eventalign ${EVENTALIGN_TSV} --out_dir ${M6ANET_DATAPREP} --n_processes 4 || die "m6anet dataprep failed"
	m6anet inference --input_dir ${M6ANET_DATAPREP} --out_dir ${M6ANET_INFERENCE} --pretrained_model arabidopsis_RNA002 --n_processes 4 --num_iterations 1000 || die "m6anet inference failed"
	deactivate

	awk -F',' '{print $1"\t"$2"\t"$2+1}' "${M6ANET_INFERENCE}/data.site_proba.csv" | tail -n +2 > ${METH_BED} || die "creating meth bed file failed"

}

fetch_reads_with_mods() {
	SITE_NO=451
	awk -F',' -v var="${SITE_NO}" '{ if($2==var){printf "%.0f\n", $3}}' "${M6ANET_INFERENCE}/data.indiv_proba.csv" | sort -nk 1 | uniq > "${OUTPUT_DIR}/read_index.list" || die "awk failed"
	cut -f1,2 ${EVENTALIGN_SUM} | tail -n +2 > "${OUTPUT_DIR}/read_id.mapping" || die "cut failed"
	awk 'NR==FNR{a[$0];next} ($1 in a)' "${OUTPUT_DIR}/read_index.list" "${OUTPUT_DIR}/read_id.mapping" | cut -f2 > ${READ_ID_LIST_MODIFIED} || die "awk filtering failed"
	awk 'NR==FNR{a[$0];next} !($1 in a)' "${OUTPUT_DIR}/read_index.list" "${OUTPUT_DIR}/read_id.mapping" | cut -f2 > ${READ_ID_LIST_NOT_MODIFIED} || die "awk filtering failed"
	head -n 3 ${READ_ID_LIST_MODIFIED} > "${READ_ID_LIST_TO_PLOT}"
	head -n 20 ${READ_ID_LIST_NOT_MODIFIED} >> "${READ_ID_LIST_TO_PLOT}"
}
...
plot_eventalign_and_sim() {
	...
	echo "squigualiser plot_pileup --read_list ${READ_ID_LIST_TO_PLOT} --bed ${METH_BED} --rna -f ${REFERENCE} -s ${SIGNAL_FILE} -a ${EVENTALIGN_BAM} --region ${REF_REGION} --tag_name HEK293T-METTL3-KO-rep1 --plot_limit 20  --profile ${PROFILE_TO_DETERMINE_BASE_SHIFT} ${SIG_SCALE}" >> ${TRACK_COMMAND_FILE}
	echo "squigualiser plot_pileup --rna -f ${SIMULATE_REF_DIR}/ref.fasta -s ${SIMULATE_REF_DIR}/sim.slow5 -a ${SIMULATE_REF_DIR}/sim.bam --region ${SIM_REGION} --tag_name ${MODEL_TO_USE}_sim_read --no_overlap --profile ${PROFILE_TO_DETERMINE_BASE_SHIFT}" >> ${TRACK_COMMAND_FILE}
